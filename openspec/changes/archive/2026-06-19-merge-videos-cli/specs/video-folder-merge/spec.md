...
When `-o` is not specified, the system SHALL create an `output` directory as a sibling of the input folder (i.e. `<input_folder>/../output/`) if it does not exist, and write the merged file as `mergedYYYYMMDDHHmmss.mp4` using the current local timestamp.

#### Scenario: Default output

- **WHEN** user runs `mergevideo ./clips` without `-o` at 2026-06-19 12:09:30
- **THEN** output is written to `./output/merged20260619120930.mp4`
